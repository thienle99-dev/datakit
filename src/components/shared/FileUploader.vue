...
const acceptedFileTypes = computed(
  () => props.accept || ".csv,.xlsx,.xls,.txt",
);
const displayDescription = computed(
  () => props.description || "Supports .csv, .xlsx, .xls",
);
...
<template>
  ...
  >
    ...
    <div>
      ...
      <p class="text-muted-foreground mt-1">{{ displayDescription }}</p>
      <button
        class="mt-4 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors pointer-events-none"
      >
        Browse Files
      </button>
    </div>
